...
class R3GANLoss:
    def __init__(self, G, D, augment_pipe=None, rank_loss=False, rank_K=8, rank_loss_type='listmle',
                 ...
                 rank_augment=True, rank_margin=1.0, rank_score_reg=0.0,
                 use_r1_penalty=True, use_r2_penalty=True, rank_replaces_adv=False):
        self.G = G
        self.D = D
        ...
        self.rank_replaces_adv = rank_replaces_adv
    ...
    def _accumulate_D_gradients_rank_based(self, real_img, real_c, gen_z, gamma, gain):
        bsz = real_img.shape[0]
        k = self.rank_K
        dtype = real_img.dtype
        device = real_img.device

        real_img = real_img.detach().requires_grad_(self.use_r1_penalty)
        rank_fake_img = self.G(gen_z, real_c).detach().requires_grad_(self.use_r2_penalty)
        rank_imgs = make_rank_list(
            real_img, rank_fake_img,
            k=k, mode=self.rank_mode, alpha_dist=self.rank_alpha_dist
        )
        rank_imgs_flat = rank_imgs.reshape(bsz * k, *rank_imgs.shape[2:])
        rank_c = real_c.repeat_interleave(k, dim=0)
        rank_imgs_to_score = self.preprocessor(rank_imgs_flat) if self.rank_augment else rank_imgs_flat
        rank_logits = self.D(rank_imgs_to_score, rank_c)
        ...
        loss_Drank = (
            listmle_loss(rank_scores) if self.rank_loss_type == 'listmle'
            else pairwise_logistic_loss(rank_scores) if self.rank_loss_type == 'pairwise_logistic'
            else pairwise_hinge_loss(rank_scores, margin=self.rank_margin) if self.rank_loss_type == 'pairwise_hinge'
            else listmle_loss(rank_scores)
        )
        ...
        R1Penalty = torch.zeros(bsz, device=device, dtype=dtype)
        R2Penalty = torch.zeros(bsz, device=device, dtype=dtype)
        if self.use_r1_penalty and rank_imgs_to_score.requires_grad:
            grad_out = torch.autograd.grad(
                rank_logits[:, 0].sum(), rank_imgs_to_score, create_graph=True
            )[0]
            R1Penalty = grad_out[0::k].square().sum([1, 2, 3])
        if self.use_r2_penalty and rank_imgs_to_score.requires_grad:
            grad_out = torch.autograd.grad(
                rank_logits[:, -1].sum(), rank_imgs_to_score, create_graph=True
            )[0]
            R2Penalty = grad_out[k - 1::k].square().sum([1, 2, 3])

        r1_term = (gamma / 2) * R1Penalty.mean() if self.use_r1_penalty else torch.tensor(0.0, device=device, dtype=dtype)
        r2_term = (gamma / 2) * R2Penalty.mean() if self.use_r2_penalty else torch.tensor(0.0, device=device, dtype=dtype)
        d_total = self.lambda_rank * loss_Drank + r1_term + r2_term
        (gain * d_total).backward()
        ...
        training_stats.report('Loss/D/total', d_total)

    def _accumulate_D_gradients_relativistic(self, real_img, real_c, gen_z, gamma, gain):
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gamma, gain):
        ...
        # D
        if phase == 'D':
            if self.rank_replaces_adv:
                self._accumulate_D_gradients_rank_based(real_img, real_c, gen_z, gamma, gain)
            else:
                self._accumulate_D_gradients_relativistic(real_img, real_c, gen_z, gamma, gain)
#----------------------------------------------------------------------------
